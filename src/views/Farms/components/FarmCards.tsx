...
const FarmCards: React.FC = () => {
  ...
  const meatIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'MEAT',
  )

  const meatPrice =
    meatIndex >= 0 && stakedValue[meatIndex]
      ? stakedValue[meatIndex].tokenPriceInWeth
      : new BigNumber(0)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [meat, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'MEAT' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
